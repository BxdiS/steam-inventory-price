...
async def get_item_price(session, appid, market_name, DEBUG=False):
    ...
    retry_delays = [10, 60, 300, 600, 1200]
    ...
    for attempt, delay in enumerate(retry_delays, start=1):
        try:
            async with session.get(url) as resp:
                ...
                if data.get('success') and not data.get('lowest_price'):
                    if DEBUG:
                        print(f"[DEBUG] {market_name} has no market price, returning 0")
                    return 0.0
        ...
        if attempt < len(retry_delays):
            if DEBUG:
                print(f"[DEBUG] Empty or failed response, retrying in {delay} seconds...")
